...
const globalLimiter = rateLimit({
  ...
  max: 200, // maks 200 request per 15 menit
  message: { error: 'Terlalu banyak request, coba lagi nanti' },
  standardHeaders: true,
  legacyHeaders: false,
});

// Rate limiting lebih ketat untuk admin/login
const authLimiter = rateLimit({
  windowMs: 15 * 60 * 1000,
  max: 20, // maks 20 percobaan login per 15 menit
...
// Terapkan lebih ketat ke route login/admin
app.use('/admin/', authLimiter);
app.use('/api/login', authLimiter);
// KEAMANAN: CORS terbatas (bisa disesuaikan)
app.use(cors({
...
// KEAMANAN: Blokir akses ke folder data
...
// KEAMANAN: Password sudah di-hash (hash dari "andalan123")
const hashedPassword = '$2a$10$N9qo8uLOickgx2ZMRZoMy.MrZQ5N5g7E6jR7X8Y9zA1b2C3d4E5fG';

// Data default dengan password ter-hash
const defaultData = {
  proyek: [
...
// KEAMANAN: Middleware autentikasi sederhana
function auth(req, res, next) {
    const authHeader = req.headers['authorization'];
    if (!authHeader || !authHeader.startsWith('Bearer ')) {
        return res.status(401).json({ error: 'Unauthorized - Token required' });
    }
    // Token sederhana (ganti dengan yang lebih aman jika perlu)
...
app.post('/api/login', async (req, res) => {
  ...
  if (user && await bcrypt.compare(password, user.password)) {
    // KEAMANAN: Jangan kirim password kembali
...
// API Proyek (dengan auth opsional untuk method POST/PUT/DELETE)
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log('✅ Keamanan: Helmet aktif | Password ter-hash | Folder data terproteksi');
});
